...
partners_df = cascades_df_with_inputs.loc[:, [f'ds_partners_{hops}hop']]

# convert to pair_ids; did this as a stop-gap because expanding the recurrent partners took too long on my current compute
partners_df[f'ds_partners_{hops}hop'] = [list(np.intersect1d(x, partners_df.index)) for x in partners_df.loc[:, f'ds_partners_{hops}hop']]

# identify upstream neurons, takes a few minutes
partners_df[f'us_partners_{hops}hop'] = generate_us_df(partners_df, hops=hops)

# identify recurrent partners
partners_df[f'recurrent_partners_{hops}hop'] = identify_recurrent_partners(partners_df, hops=hops)
...
MBIN_partners.loc[:, f'recurrent_partners_{hops}hop'] = [Promat.get_paired_skids(skids, pairs, unlist=True) for skids in list(MBIN_partners.loc[:, f'recurrent_partners_{hops}hop'])]

MBIN_rec_partner_cts = [Celltype(f'{MBIN_partners.index[i]}_recurrent-partners', skids) for i, skids in enumerate(list(MBIN_partners.loc[:, f'recurrent_partners_{hops}hop']))]
...
MBIN_rec_partner_cta.plot_memberships(path=f'plots/MBIN_recurrent-partners-{hops}hops.pdf', figsize=(3,1))
MBIN_rec_partner_cta.plot_memberships(path=f'plots/MBIN_recurrent-partners-{hops}hops_raw-num.pdf', figsize=(3,1), raw_num=True)

# %%
# how many sensory neurons from each modality talk to MBINs within X-hops?
...
MBIN_partners.loc[:, f'us_partners_{hops}hop'] = [Promat.get_paired_skids(skids, pairs, unlist=True) for skids in list(MBIN_partners.loc[:, f'us_partners_{hops}hop'])]

MBIN_us_partner_cts = [Celltype(f'{MBIN_partners.index[i]}_us-partners', skids) for i, skids in enumerate(list(MBIN_partners.loc[:, f'us_partners_{hops}hop']))]
...
MBIN_us_partner_cta.plot_memberships(path=f'plots/MBIN_us-sens-partners-{hops}hops.pdf', figsize=(3,1))
MBIN_us_partner_cta.plot_memberships(path=f'plots/MBIN_us-sens-partners-{hops}hops_raw-num.pdf', figsize=(3,1), raw_num=True)
...
# what is actually used in the paper
...
(MBINs_sens_input>=(n_init/2)).sum(axis=0)
