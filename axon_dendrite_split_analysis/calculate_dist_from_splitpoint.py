...
connectors = pd.read_csv('axon_dendrite_data/splittable_connectors_all_mod.csv', header=0, skipinitialspace=True, keep_default_na = False)
skeletons = pd.read_csv('axon_dendrite_data/splittable_skeletons_all_mod.csv', header=0, skipinitialspace=True, keep_default_na = False)
...
for i in tqdm(range(len(skids))):
    if(sum(splits['skeleton'] == skids[i])>=1):
        split_ids.append(splits['treenode'][splits['skeleton'] == skids[i]].values[0])
    if(sum(splits['skeleton'] == skids[i])==0):
        print('skid %i has no split point' % skids[i])


print(len(skeleton_graphs))
print(len(list_connectors))
print(len(roots))
print(len(split_ids))
