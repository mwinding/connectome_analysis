...
density = [f'{G_names[i]}: {round(density[i], 2)}%' for i in range(0, len(density))]
...
synapse_counts = [list(pg.Prograph.path_edge_attributes(G, edge, 'weight')[0]) + [G_names[i]] for i, G in enumerate(G_types) for edge in list(G.edges)]
...
plt.savefig('small_plots/plots/treemap_fraction-edges_fraction-synapses.pdf', format='pdf', bbox_inches='tight')
...
plt.savefig('small_plots/plots/treemap_count-edges_count-synapses.pdf', format='pdf', bbox_inches='tight')
# %%
